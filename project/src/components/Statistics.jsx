...
import { Box, Card, CardContent, Typography, Grid, Paper, Alert } from '@mui/material';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
function Statistics({ mode }) {
  ...
  if (error) {
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="error">Failed to load statistics: {error.message}</Alert>
      </Box>
    );
  }
  ...
  const emissionsByAircraft = flights.reduce((acc, flight) => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper elevation={2} sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>CO2 Emissions by Aircraft</Typography>
            <Box sx={{ height: 300 }}>
              <Bar data={co2ByAircraftData} options={co2ByAircraftOptions} />
            </Box>
          </Paper>
        </Grid>
      </Grid>
